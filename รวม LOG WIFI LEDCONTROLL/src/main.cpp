...
void setup() {
    ...
    server.on("/setMode", HTTP_GET, []() {
        ...
        if (mode == "ON" || mode == "OFF" || mode == "AUTO") {
            ledMode = mode;
            if (mode == "ON") {
                digitalWrite(LED_PIN, LOW);  // Changed to LOW for ON
            } else if (mode == "OFF") {
                digitalWrite(LED_PIN, HIGH); // Changed to HIGH for OFF
            }
        }
    ...
    server.on("/getLedStatus", HTTP_GET, []() {
        bool currentState = digitalRead(LED_PIN);
        server.send(200, "text/plain", currentState == LOW ? "ON" : "OFF");
    });
...
void loop() {
    ...
    if (ledMode == "AUTO") {
        ...
        if (currentMillis - previousMillis >= blinkInterval) {
            ...
            ledState = !ledState;
            digitalWrite(LED_PIN, ledState ? HIGH : LOW);  // Inverted logic
        }
    }
}